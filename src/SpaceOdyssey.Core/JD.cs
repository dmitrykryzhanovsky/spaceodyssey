namespace SpaceOdyssey
{
    ...
    public static class JD
    {
        ...
        {
            (int jdnCommonTerm, _) = JDNCommonTerm (year, month, day);
        ...
        {
            return (sec + TimeUnits.SecInMin * (min + TimeUnits.MinInHour * hour)) / TimeUnits.SecInDay;
        }
        ...
        {
            return (double)(millisec + TimeUnits.MillisecInSec * 
                                      (sec + TimeUnits.SecInMin * 
                                            (min + TimeUnits.MinInHour * hour))) / TimeUnits.MillisecInDay;
        }
        ...
        {
            double totalSecF   = dayFraction * TimeUnits.SecInDay;
            int    totalSecInt = (int)totalSecF;

            (int totalMin, int secInt) = int.DivRem (totalSecInt, TimeUnits.SecInMin);
            (int hour, int min) = int.DivRem (totalMin, TimeUnits.MinInHour);
        ...
        /// </para>
        /// </remarks>
        public static double GetJulianCentirues (double t, double t0 = Epochs.J2000)
        {
            return (t - t0) / TimeUnits.JulianCentury;
        }
    }
}